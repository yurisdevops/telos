...
import { limparTreinosProntosOrfaos } from './ready-workouts';
 ...
 * exercícios pulados, e os planos `'Treino pronto'` efêmeros que ficam
 * órfãos quando as sessões deles somem. Numa única transação síncrona: ou
 * apaga tudo, ou (erro no meio) reverte por completo, banco fica exatamente
 * como estava — mesmo padrão de `treinarAgora`/`importBackupPayload`.
 *
 * NÃO toca em: `workoutPlans`/`workoutDays`/`workoutDayExercises` de tipo
 * diferente de `'Treino pronto'` (planos normais e `'Pronto'` salvos como
 * planilha continuam intactos), `bodyWeightLogs` (peso), `userProfile`
 * (perfil inteiro, **incluindo `pinHash`/`pinSalt`** — o PIN sobrevive ao
 * reset, precisa continuar valendo pro próximo reset), `exercises`
 * (catálogo). Nenhuma dessas tabelas é mencionada abaixo de propósito.
 */
export function resetHistory(): void {
  db.transaction((tx: Tx) => {
    ...
    // Só DEPOIS de `sessions` estar vazia: reaproveita a limpeza de treinos
    // prontos órfãos (ready-workouts.ts) — com nenhuma sessão sobrando, todo
    // plano `'Treino pronto'` fica sem referência nenhuma ao seu dia, então
    // a função apaga todos eles (e só eles — filtra estritamente por
    // `tipo === 'Treino pronto'`, nunca toca em planos normais/'Pronto').
